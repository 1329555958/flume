 ...
 * under the License.
 */
package com.vf.flume.sink.elasticsearch2;
...
import com.vf.flume.sink.elasticsearch2.client.ElasticSearchClient;
import com.vf.flume.sink.elasticsearch2.client.ElasticSearchClientFactory;
    ...
    try {
      ...
      for (count = 0; count < batchSize; ++count) {
        ...
        if (++eventCounter % 10000 == 0) {
            logger.info("es sink {} successful processed {} events.", getName(), eventCounter );
//            logger.info(eventCounter+" events.body  {}  headers {}", new String(event.getBody()) , event.getHeaders());
          }
        if (event == null) {
          break;
        }
      ...
      counterGroup.addAndGet("events.success", (long) Math.min(batchSize, count));
